...
export function Sidebar() {
  const isAdmin = CURRENT_USER.role === UserRole.ADMIN
  const location = useLocation()
  ...
  return (
    <aside className="w-60 shrink-0 bg-slate-900 flex flex-col h-screen sticky top-0">
      ...
      <nav className="flex-1 overflow-y-auto scrollbar-thin px-3 py-4 flex flex-col gap-1">
        {NAV.map((item) => (
          <NavGroup key={item.path} item={item} open={!!openGroups[item.path]} onToggle={() => toggle(item.path)} />
        ))}
      ...
      <div className="px-3 py-3 border-t border-slate-800">
        <div className="flex items-center gap-2.5 px-2 py-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-slate-200 text-xs font-medium truncate">{CURRENT_USER.displayName}</p>
            <p className="text-slate-500 text-xs truncate capitalize">{CURRENT_USER.role.toLowerCase()}</p>
          </div>
